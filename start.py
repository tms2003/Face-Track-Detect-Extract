...
def main():
    ...
    ampl_rate=1/scale_rate # to zoom rate
        ...
        with tf.Session(config=tf.ConfigProto(gpu_options=tf.GPUOptions(allow_growth=True),
            ...
            for filename in os.listdir(videos_dir):
                ...
                while True:
                    ...
                    ret, frame_bak= cam.read()
                    if not ret:
                        logger.warning("ret false")
                        break
                    if frame_bak is None:
                        logger.warning("frame drop")
                        break

                    frame = cv2.resize(frame_bak, (0, 0), fx=scale_rate, fy=scale_rate)
                        ...
                        if face_sums > 0:
                            ...
                            for i, item in enumerate(faces):
                                ...
                                if score > face_score_threshold:
                                    ...
                                    # face cropped
                                    #bb_bak = np.array(det, dtype=np.int32)
                                    bb=np.array(det* ampl_rate, dtype=np.int32)
                                    ...
                                    cropped = frame_bak[bb[1]:bb[3], bb[0]:bb[2], :].copy()
...
def parse_args():
    ...
    parser.add_argument("--videos_dir", type=str,
                        help='Path to the data directory containing aligned your face patches.', default='/home/incar/data/video2')
